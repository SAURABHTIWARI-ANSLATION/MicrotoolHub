// src/index.tsx (Converted from App.jsx)
...
const getDailyData = (dateISO: string) => {
...
const CalendarPanchang = () => {
    // State for the selected date 
    const todayISO = new Date().toISOString().split('T')[0];
    const [selectedDate, setSelectedDate] = useState(todayISO); 
    
    // State for the actual panchang data displayed
    const [panchangData, setPanchangData] = useState<any>(getDailyData(todayISO));
    ...
    const majorOccasionDisplay = panchangData.festivals.find((f: string) => f.includes('Major') || f.includes('Ekadashi')) || panchangData.festivals[panchangData.festivals.length - 1] || "N/A";


    // --- JSX Rendering ---
    return (
        <div className="panchang-app">
            {/* Using the main app's header instead of a custom one */}
            <div className="container">
                    ...
                    {!loading && panchangData && (
                        <>
                            ...
                            <div className="card festival-card">
                                ...
                                <ul id="festivalList">
                                    {panchangData.festivals.map((festival: string, index: number) => (
                                        <li 
                                            key={index}
                ...
                <p style={{textAlign:'center', fontStyle: 'italic', marginTop:'20px'}}>Disclaimer: Data accuracy depends on the API and location settings. (Default: UTC/Generic)</p>

            </div>

            <footer className="footer">
                <p>&copy; 2025 Calendar Panchang Tool. Crafted with care.</p>
...
export default CalendarPanchang;