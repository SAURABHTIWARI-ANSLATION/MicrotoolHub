...
export function pushHistory(item) {
  try {
    ...
    const isDuplicate = arr.slice(0, 5).some(existing => 
...
export function getHistoryStats() {
  ...
  return {
    ...
    categoryStats,
    oldestConversion: history.length > 0 ? Math.min(...history.map(h => h.ts)) : null,
    newestConversion: history.length > 0 ? Math.max(...history.map(h => h.ts)) : null
  }
}
...
export function loadRounding() {
  try {
    ...
    if (parsed && (parsed.mode === 'auto' || parsed.mode === 'fixed')) return parsed
    return { mode: 'auto' }
  } catch {
    return { mode: 'auto' }
  }
